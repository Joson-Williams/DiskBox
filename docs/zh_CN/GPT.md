...
**扇区编辑支持**</br>
DiskBox提供了可以访问并修改磁盘所有扇区的功能。</br>DiskBox不提倡由用户手动编辑分区表，除非这是必须行动；同时DiskBox也不禁止用户的手动编辑行为。</br>出于安全考虑，当用户尝试手动编辑分区表数据时，软件会发出提示。但软件不会拦截编辑行为本身，也不对因手动编辑分区表造成的数据丢失作出任何明示或暗示的担保。
